...
class MapComp extends Component {
    constructor(props) {
        ...
        this.onchange = (e) =>{
            ...
            const newContent = {
                ...this.state.inputContent, // inputContent에 값 다불러옴
                [e.target.name] : e.target.value // input으로 작성한내용 출력
            }
    ...
    render() { 
        ...
        // 한번만 쓸거면 season.map 그대로 리턴에 넣어도됨
        const seasonItem = season.map(  // index : 0,1,2,3 배열
        ...
        const bookItem = book.map(
            (book, index)=> (
                // book.title을 클릭하면 삭제
                <li onClick={()=>{
                    const newTitles = this.state.book.filter((b)=>b.number!=book.number)
                    this.setState({book:newTitles});
                }} key={book.number}>{book.title}</li>
                )
            )
